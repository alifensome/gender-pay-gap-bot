...
export class LambdaClient {
  ...
  trigger({
    ...
    return new Promise((resolve, reject) => {
      ...
      this.awsLambda.invoke(
        params,
        (err: AWSError, data: Lambda.InvocationResponse) => {
          if (err) {
            this.logger.error({
              message: "error while triggering lambda",
          ...
          if (data.StatusCode !== 200 && data.StatusCode !== 201) {
            this.logger.error({
              message: "expected status code 200 or 201",
              statusCode: data.StatusCode,
              logs: base64ToString(data.LogResult),
            });
            return reject(data);
          }
